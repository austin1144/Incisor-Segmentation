...
def __get_colors(num_colors):
    ...
    """
    colors = []
    for i in np.arange(0., 360., 360./num_colors):  # https://docs.scipy.org/doc/numpy/reference/generated/numpy.arange.html
        #print(i)
        hue = i/360.
        lightness = (50 + np.random.rand() * 10)/100.
        saturation = (90 + np.random.rand() * 10)/100.
        colors.append(colorsys.hls_to_rgb(hue, lightness, saturation))
   # print([(int(r*255), int(g*255), int(b*255)) for (r, g, b) in colors])
    return [(int(r*255), int(g*255), int(b*255)) for (r, g, b) in colors]

def drawlines(img, lm):
    #print len(lm)
